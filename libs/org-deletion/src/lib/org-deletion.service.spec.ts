...
describe('OrgDeletionService', () => {
  ...
  describe('requestDeletion', () => {
    it('throws NotFoundException when organization does not exist', async () => {
      ...
      await expect(
        service.requestDeletion(ORG_UUID, DeletionTrigger.USER_REQUEST, USER_UUID),
    ...
    it('uses default retention period from config when org has no custom period', async () => {
      ...
      await service.requestDeletion(ORG_UUID, DeletionTrigger.USER_REQUEST, USER_UUID);

      const updateCall = prisma.organization.update.mock.calls[0][0];
      const scheduledAt = updateCall.data.deletionScheduledAt as Date;
      const requestedAt = updateCall.data.deletionRequestedAt as Date;
    ...
    it('updates org status to PENDING_DELETION and sets timestamps', async () => {
      ...
      expect(updateCall.data.status).toBe(OrganizationStatus.PENDING_DELETION);
      expect(updateCall.data.deletionRequestedAt).toBeInstanceOf(Date);
      expect(updateCall.data.deletionScheduledAt).toBeInstanceOf(Date);
    });
    ...
    it('allows undefined userId for system-triggered deletions', async () => {
      ...
      expect(eventCall.payload.trigger).toBe(DeletionTrigger.SUBSCRIPTION_EXPIRY);
    });
  });
});
