...
describe('OrgDeletionSchedulerService', () => {
  ...
  describe('checkExpiredOrganizations', () => {
    it('finds SUSPENDED orgs with deletionScheduledAt in the past', async () => {
      ...
      const findManyCall = prisma.organization.findMany.mock.calls[0][0];
      expect(findManyCall.where.status).toBe('SUSPENDED');
      expect(findManyCall.where.deletionScheduledAt.lte).toBeInstanceOf(Date);
    ...
    it('triggers deletion for each expired organization', async () => {
      ...
      prisma.organization.findMany.mockResolvedValueOnce([org1, org2]);
    ...
    it('passes undefined userId for automated deletions', async () => {
      ...
      prisma.organization.findMany.mockResolvedValueOnce([org]);
    ...
    it('does not throw when findMany fails', async () => {
      ...
      await expect(
        service.checkExpiredOrganizations(),
